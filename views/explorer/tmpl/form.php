...
/**
 * Copyright (c) 2006-2016 LogicalDOC
...
<script language="javascript" type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        jQuery("#searchAdvanced").button().click(function () {
            ...
            var palabraClave = jQuery('#palabraClave').val();
            if (contenido != '' || nombre != '' || palabraClave != '') {
                document.formSearchAdvanced.task.value = 'searchAdvanced';
                document.formSearchAdvanced.submit();
            }
            else {
                jQuery('#nombre').validationEngine('showPrompt', "<?php _e('Some of these fields are required', 'codended');?>", 'error', true);
                return false;
            }
        });
...
<form action="" method="GET" name="formSearchAdvanced"
...
<?php
   if (empty($_GET["contenido"])) {
       echo "<input type='text' name='contenido' id='contenido' value=''/>";
   } else {
       $contentFiltered = filter_var($_GET["contenido"], FILTER_SANITIZE_STRING);
       echo "<input type='text' name='contenido' id='contenido' value='" .$contentFiltered ."'/>";
   }
?>
                    </td>
                </tr>
